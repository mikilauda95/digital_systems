...
#define COMPATIBLE_STRING	"xlnx,sha256-wrap-1.0"

/* information about kernel module */
MODULE_AUTHOR("Michele Simili");
MODULE_LICENSE("GPL");
...
/* called on device read. read 32*2 bytes (data and status registers). */
static ssize_t sha256_read(struct file *fp, char *buf, size_t len, loff_t *off)
{
    uint32_t data_r; // read data

    rmb();

	//Read a 32 bits register
    data_r = ioread32(base_addr + *off);

    if(copy_to_user(buf, &data_r, sizeof(uint32_t))) // copy read data to user space
    {
        return -EACCES; // if error return -EACCES error code
    }
    return sizeof(len); // return number of read bytes
}

// USE THE OFFSET TO DECIDE WHERE TO WRITE (status vs data) and the length to decide if to write status or data register.
static ssize_t sha256_write(struct file *fp, const char __user *buf, size_t len, loff_t *off)
{
    uint32_t data_w; // write data
    char block[4];

    if (copy_from_user(&block, buf, sizeof(uint32_t))) {
        return -EFAULT;
    } 
    else {

        data_w =  ((uint32_t)block[0] << 24);
        data_w |= ((uint32_t)block[1] << 16);
        data_w |= ((uint32_t)block[2] << 8);
        data_w |= ((uint32_t)block[3]); 

		// Write the 32 bits in the register
        iowrite32(data_w, base_addr + *off);
        return len;
    }
}
